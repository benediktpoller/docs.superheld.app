...
> `TODO-ENG-044`: Feature-Vektor-Spezifikation (Feature-Liste, Dimensionalität, Transformationsverfahren) dokumentieren. `TODO-ENG-045`: Differential-Privacy-Parameter (ε, δ) bestätigen oder DP-Sprache entfernen. Siehe [Privacy Model](/experts/spec/privacy-model).
...
> `TODO-ENG-034`: Retention-Zeiträume bestätigen (90d lokal, 180d Cloud, 12 Monate Audit, 24 Monate Metriken). Siehe [Event Pipeline](/experts/spec/event-pipeline).
...
> `TODO-ENG-032`: Webhook-Header (`X-Superheld-Timestamp`, Idempotency-Key) und Retry-Verhalten bestätigen. Siehe [Event Pipeline](/experts/spec/event-pipeline).
