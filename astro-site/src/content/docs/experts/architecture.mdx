...
- **Model Update Service:** Verteilt aktualisierte ML-Modelle als signierte Pakete an die lokalen Schutz-Engines. Rollback-Schutz: der Agent lehnt ältere Modellversionen ab. `TODO-ENG-051`: Signaturalgorithmus und Schlüsselmanagement mit Engineering bestätigen. `TODO-ENG-016`: Update-Frequenz bestätigen.
- **AI Analysis Module:** GPU-gestützte Analyse für komplexe Fälle (Deepfake-Erkennung, fortgeschrittene Social-Engineering-Analyse). Verarbeitet ausschließlich anonymisierte Feature-Vektoren — niemals Klartext. `TODO-ENG-044`: Genaue Feature-Vektor-Spezifikation (Merkmale, Dimensionalität, Transformationsverfahren) dokumentieren.
...
:::note
Die Sicherheitsgrenze ist kryptografisch durchgesetzt: Alle Kommunikation zwischen Gerät und Cloud ist mit TLS 1.3 + Certificate Pinning gesichert. Feature-Vektoren werden vor der Übertragung lokal transformiert. Cloud-Anfragen enthalten keine Geräte- oder Benutzer-IDs und sind nicht über Anfragen hinweg korrelierbar (`TODO-ENG-001`: Statelessness-Garantie bestätigen).
:::
...
2. **Rekonstruktionsresistenz.** Die Feature-Vektoren, die bei einer Cloud-Eskalation übertragen werden, werden durch Dimensionsreduktion und irreversible Transformationen verarbeitet. Die genauen Transformationsverfahren und formalen Garantien sind in der [Privacy-Model-Spezifikation](/experts/spec/privacy-model) dokumentiert. `TODO-ENG-044`: Transformationsverfahren und Dimensionalität bestätigen. `TODO-ENG-045`: Differential-Privacy-Parameter (ε, δ) bestätigen, falls DP angewendet wird.

3. **Modell-Updates ohne zentrale Rohdaten.** ML-Modelle werden als kryptografisch signierte Pakete an die lokalen Schutz-Engines verteilt. Der Agent verifiziert die Signatur vor dem Laden und lehnt ältere Versionen ab (Rollback-Schutz). `TODO-ENG-051`: Signaturalgorithmus bestätigen.
