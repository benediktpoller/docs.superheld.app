...
`TODO-ENG-032`: Bestätigen, ob Superheld einen `X-Superheld-Timestamp`-Header mitsendet. Falls ja, sollte dieser anstelle des Payload-Feldes für die Replay-Prüfung verwendet werden.

**Empfohlene Implementierung (Consumer-seitig):**

1. Prüfen Sie den Zeitstempel im Payload (`timestamp`-Feld) gegen die aktuelle Serverzeit.
...
`TODO-ENG-032`: Bestätigen, ob ein dedizierter `X-Superheld-Idempotency-Key`-Header gesendet wird oder ob `event_id` im Payload als einziger Idempotenz-Schlüssel dient.
...
`TODO-ENG-033`: Bestätigen, ob fehlgeschlagene Webhooks manuell aus der DLQ erneut ausgelöst werden können (Dashboard-Replay).
